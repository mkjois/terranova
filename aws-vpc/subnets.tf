...
resource "aws_subnet" "private" {
  ...
  tags {
    Name = "${var.name}-private-${element(var.az_order, count.index)}"
  }
}
...
resource "aws_subnet" "public" {
  ...
  tags {
    Name = "${var.name}-public-${element(var.az_order, count.index)}"
  }
}
...
resource "aws_subnet" "spare" {
  ...
  tags {
    Name = "${var.name}-reserved-${element(var.az_order, count.index)}"
  }
}
